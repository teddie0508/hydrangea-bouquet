...
// - Mỗi bông có kích thước ~ cố định (R), thêm bông thì cả BÓ TO/ĐẦY lên
//   (bán kính cụm ~ tỉ lệ căn(số bông)) -> tăng "Số bông" thấy rõ nhiều hoa hơn.
// - Độ xòe (spread) điều khiển độ thưa giữa các bông: nhỏ = khít, lớn = thoáng.
function makeHeads(templateId, params, rng) {
  ...
  const overlap = 0.86 + spread * 0.85 // độ thưa giữa các bông
  const disk = R * overlap * Math.sqrt(blooms) * 0.62 // bán kính cụm theo số bông
  let raw = []
  ...
  switch (templateId) {
    case 'cascade': {
      ...
      raw = domePositions(top, disk, disk * 0.82, rng)
      for (let i = 0; i < tail; i++) {
        const p = (i + 1) / (tail + 1)
        raw.push({
          x: -disk * 0.32 + (rng() - 0.5) * disk * 0.55,
          y: disk * 0.7 + p * disk * 1.5,
          depth: 0.5 + p * 0.4,
        })
      }
      break
    ...
    case 'heart':
      raw = heartPositions(blooms, disk / 13, rng)
      break
    case 'asymmetric':
      raw = domePositions(blooms, disk * 1.05, disk * 0.72, rng).map((p) => ({
        ...p,
        x: p.x + p.y * 0.22 + R * 0.2,
      }))
      break
    case 'petite':
      raw = domePositions(blooms, disk * 0.74, disk * 0.66, rng)
      break
    case 'round':
    default:
      raw = domePositions(blooms, disk, disk * 0.92, rng)
      break
  }
...
// Dời cụm hoa vào khung, neo đáy cụm vào chỗ cuống.
// Giữ nguyên kích thước tự nhiên (bông không co lại khi thêm bông);
// chỉ thu nhỏ khi cụm vượt mép khung để chống tràn.
function fitHeads(heads) {
  ...
  // chỉ thu nhỏ khi cần (cho phép phóng nhẹ tối đa 1.06 để cụm nhỏ không quá bé)
  const scale = Math.min(1.06, CLUSTER_BOX.w / w, boxH / hgt)
